...
from src.infrastructure.models import MediaOrm, UserOrm
...
async def seed():
    ...
    try:
        ...
        async with async_session() as session:
            ...
            result = await session.execute(insert(UserOrm).returning(UserOrm.id), users_data)
            ...
            result = await session.execute(insert(MediaOrm).returning(MediaOrm.id), medias_data)
            ...
            tweets_data = [
                ...
                {
                    "content": "Cat media tweet",
                    "author_id": user_ids[2],
                    "medias_id": [media_ids[4]],
                },
                {"content": "The end!", "author_id": user_ids[2], "medias_id": []},
            ]

            result = await session.execute(insert(TweetOrm).returning(TweetOrm.id), tweets_data)
